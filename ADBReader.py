...
#and (int(parts[3], 16) == 1) :
#and (int(parts[3], 16) == 0) :
...
for element in array :
        if (str(element[:18]) == "/dev/input/event0:") :
			...
			counter += 1
                        if startX == 0 : startX = lastX
                elif (int(parts[1], 16) == 3) and (int(parts[2], 16) == deviceY) :
                        lastY = int(parts[3], 16)
			counter += 1
			if startY == 0 : startY = lastY
                elif (int(parts[1], 16) == startStopPrefix) and (int(parts[2], 16) == startStop) and touchStart == False : 
                        touchStart = True
                elif (int(parts[1], 16) == startStopPrefix) and (int(parts[2], 16) == startStop) and touchStart == True: 
                        touchEnd = True
#                elif (int(parts[1], 16) == 0) and (int(parts[2], 16) == 0) and (int(parts[3], 16) == 0) :
